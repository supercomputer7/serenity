...
if [ -z "$SERENITY_QEMU_DISPLAY_DEVICE" ]; then
    if [ "$SERENITY_GL" = "1" ]; then
        # QEMU appears to not support the virtio-vga-gl VirtIO GPU variant on macOS.
        # To ensure we can still boot on macOS with VirtIO GPU, use the virtio-gpu-gl-pci
        # variant, which lacks any VGA compatibility (which is not relevant for us anyway).
        if [ "$(uname)" = "Darwin" ]; then
            SERENITY_QEMU_DISPLAY_DEVICE="virtio-gpu-gl-pci "
        else
