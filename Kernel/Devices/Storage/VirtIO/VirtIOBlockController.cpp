...
VirtIOBlockController::VirtIOBlockController(NonnullOwnPtr<VirtIO::TransportEntity> transport)
...
ErrorOr<NonnullRefPtr<VirtIOBlockController>> VirtIOBlockController::try_create(PCI::DeviceIdentifier const& device_identifier)
{
    auto transport_link = TRY(VirtIO::PCIeTransportLink::create(device_identifier));
    auto controller = TRY(adopt_nonnull_ref_or_enomem(new (nothrow) VirtIOBlockController(move(transport_link))));
    TRY(controller->initialize_virtio_resources());
    TRY(controller->attach_block_device());
    return controller;
}

ErrorOr<void> VirtIOBlockController::initialize_virtio_resources()
{
    dbgln_if(VIRTIO_DEBUG, "VirtIOBlockController::initialize_virtio_resources");
    TRY(Device::initialize_virtio_resources());
    m_header_buf = TRY(MM.allocate_contiguous_kernel_region(
        PAGE_SIZE, "VirtIOBlockController header_buf"sv, Memory::Region::Access::Read | Memory::Region::Access::Write));
    m_data_buf = TRY(MM.allocate_contiguous_kernel_region(
        INFLIGHT_BUFFER_SIZE, "VirtIOBlockController data_buf"sv, Memory::Region::Access::Read | Memory::Region::Access::Write));
...
void VirtIOBlockController::start_request(AsyncBlockDeviceRequest& request)
{
...
ErrorOr<void> VirtIOBlockController::maybe_start_request(AsyncBlockDeviceRequest& request)
{
    VERIFY(m_attached_device);
    auto& queue = get_queue(REQUESTQ);
    SpinlockLocker queue_lock(queue.lock());
    VirtIO::QueueChain chain(queue);

    u64 data_size = m_attached_device->block_size() * request.block_count();
...
void VirtIOBlockController::handle_queue_update(u16 queue_index)
{
...
void VirtIOBlockController::respond()
{
    VERIFY(m_attached_device);
...
ErrorOr<void> VirtIOBlockController::handle_device_config_change()
{
    dbgln_if(VIRTIO_DEBUG, "VirtIOBlockDevice::handle_device_config_change");
    return {};
}

ErrorOr<void> VirtIOBlockController::attach_block_device()
{
    auto lun = StorageDevice::LUNAddress { controller_id(), 0, 0 };
    auto cid = hardware_relative_controller_id();

    m_attached_device = TRY(adopt_nonnull_ref_or_enomem(new (nothrow) VirtIOBlockDevice(*this, lun, cid)));
    return {};
}
