...
void NVMeController::set_admin_q_depth()
{
...
ErrorOr<void> NVMeController::create_admin_queue(QueueType queue_type)
{
    ...
    m_admin_queue = TRY(NVMeQueue::try_create(*m_pci_device, 0, irq, ADMIN_QUEUE_SIZE, move(cq_dma_region), move(sq_dma_region), move(doorbell), queue_type));
