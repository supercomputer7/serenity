...
AHCIPort::AHCIPort(AHCIController const& controller, NonnullRefPtr<Memory::PhysicalRAMPage> identify_buffer_page, AHCI::HBADefinedCapabilities hba_capabilities, volatile AHCI::PortRegisters& registers, u32 port_index)
...
void AHCIPort::recover_from_fatal_error()
{
    ...
    dmesgln("{}: AHCI Port {} fatal error, shutting down!", m_parent_controller->pci_device().device_id().address(), representative_port_index());
    dmesgln("{}: AHCI Port {} fatal error, SError {}", m_parent_controller->pci_device().device_id().address(), representative_port_index(), (u32)m_port_registers.serr);
