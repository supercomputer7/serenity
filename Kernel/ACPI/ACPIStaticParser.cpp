...
ACPI_RAW::SDTHeader* ACPIStaticParser::find_table(const char* sig)
{
    ...
    for (auto* physical_sdt_ptr : m_main_sdt->get_sdt_pointers()) {
        // FIXME: Remove Region::Access::Write
        auto region = MM.allocate_kernel_region((PAGE_SIZE * 2), "ACPI Static Parser Tables Finding", Region::Access::Read | Region::Access::Write);
...
void ACPIStaticParser::init_fadt()
{
    ...
    m_fadt = adopt(*new ACPI::FixedACPIData(*((ACPI_RAW::FADT*)(fadt_region->vaddr().get() + ((u32)(&physical_fadt) & (~PAGE_MASK))))));
#ifdef ACPI_DEBUG
...
{
    OwnPtr<Region> checkup_region = MM.allocate_kernel_region((PAGE_SIZE * 2), "ACPI Static Parser Copying Method", Region::Access::Read);
#ifdef ACPI_DEBUG
    dbgprintf("ACPI: Checking Main SDT Length to choose the correct mapping size");
#endif
    mmap_region(*checkup_region, PhysicalAddress((u32)m_main_system_description_table.ptr() & PAGE_MASK));

    auto* sdt = (ACPI_RAW::SDTHeader*)(checkup_region->vaddr().get() + ((u32)m_main_system_description_table.ptr() & (~PAGE_MASK)));
    u32 length = sdt->length;
    ...
    mmap_region(*main_sdt_region, PhysicalAddress((u32)m_main_system_description_table.ptr() & PAGE_MASK));

    Vector<ACPI_RAW::SDTHeader*>* sdts_pointer = new Vector<ACPI_RAW::SDTHeader*>();
    if (m_xsdt_supported) {
        ACPI_RAW::XSDT* xsdt = (ACPI_RAW::XSDT*)(main_sdt_region->vaddr().get() + ((u32)m_main_system_description_table.ptr() & (~PAGE_MASK)));
        kprintf("ACPI: Using XSDT, Enumerating tables @ P 0x%x\n", m_main_system_description_table.ptr());
...
#endif
            sdts_pointer->append((ACPI_RAW::SDTHeader*)xsdt->table_ptrs[i]);
        }
    } else {
        ACPI_RAW::RSDT* rsdt = (ACPI_RAW::RSDT*)(main_sdt_region->vaddr().get() + ((u32)m_main_system_description_table.ptr() & (~PAGE_MASK)));
        ...
        for (u32 i = 0; i < ((rsdt->h.length - sizeof(ACPI_RAW::SDTHeader)) / sizeof(u32)); i++) {
            ...
            dbgprintf("ACPI: Found new table, @ P 0x%x\n", rsdt->table_ptrs[i]);
#endif
            sdts_pointer->append((ACPI_RAW::SDTHeader*)rsdt->table_ptrs[i]);
        }
    }
    m_main_sdt = new ACPI::MainSystemDescriptionTable(*sdts_pointer);
}
...
void ACPIStaticParser::locate_all_aml_tables()
{
    ...
    for (auto* sdt_ptr : m_main_sdt->get_sdt_pointers()) {
        ...
        ACPI_RAW::SDTHeader* sdt = (ACPI_RAW::SDTHeader*)(region->vaddr().get() + ((u32)sdt_ptr & (~PAGE_MASK)));
#ifdef ACPI_DEBUG
        dbgprintf("ACPI: Examining Table @ P 0x%x\n", sdt_ptr);
#endif
...
ACPIStaticParser::ACPIStaticParser()
    ...
    , m_mcfg(nullptr)
    , m_madt(nullptr)
    , m_aml_tables_ptrs(*new Vector<ACPI_RAW::SDTHeader*>())
{
    m_rsdp = search_rsdp();
    if (m_rsdp != nullptr) {
...
OwnPtr<ACPI_RAW::RSDPDescriptor20> ACPIStaticParser::search_rsdp()
{
    ...
    for (char* rsdp_str = (char*)(PhysicalAddress(ebda_seg << 4).as_ptr()); rsdp_str < (char*)((ebda_seg << 4) + 1024); rsdp_str += 16) {
        ...
        if (!strncmp("RSD PTR ", rsdp_str, strlen("RSD PTR ")))
            return OwnPtr<ACPI_RAW::RSDPDescriptor20>((ACPI_RAW::RSDPDescriptor20*)rsdp_str);
    }
