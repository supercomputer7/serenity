...
{
    auto registers_mapping = TRY(Memory::map_typed<volatile PlaneRegisters>(plane_registers_start_address, sizeof(PlaneRegisters), Memory::Region::Access::ReadWrite));
    return adopt_nonnull_own_or_enomem(new (nothrow) IntelG33DisplayPlane(move(registers_mapping)));
}

IntelG33DisplayPlane::IntelG33DisplayPlane(Memory::TypedMapping<volatile PlaneRegisters> registers_mapping)
...
ErrorOr<void> IntelG33DisplayPlane::enable(Badge<IntelDisplayConnectorGroup>)
{
    ...
    VERIFY(m_aperture_start.get() < UINT32_MAX);
