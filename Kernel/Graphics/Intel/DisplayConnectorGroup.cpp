...
NonnullRefPtr<IntelDisplayConnectorGroup> IntelDisplayConnectorGroup::must_create(Badge<IntelNativeGraphicsAdapter>, IntelGraphics::Generation generation, MMIORegion const& first_region, MMIORegion const& second_region)
{
    ...
    auto connector_group = adopt_ref_if_nonnull(new (nothrow) IntelDisplayConnectorGroup(generation, address_space.release_nonnull(), move(gmbus_connector), move(registers_region), first_region, second_region)).release_nonnull();
    MUST(connector_group->initialize_connectors());
    return connector_group;
}

IntelDisplayConnectorGroup::IntelDisplayConnectorGroup(IntelGraphics::Generation generation, NonnullOwnPtr<IntelGraphics::AddressSpace> address_space, NonnullOwnPtr<GMBusConnector> gmbus_connector, NonnullOwnPtr<Memory::Region> registers_region, MMIORegion const& first_region, MMIORegion const& second_region)
...
ErrorOr<void> IntelDisplayConnectorGroup::initialize_gen4_connectors()
{
    ...
    m_transcoders[0] = MUST(IntelCRTDisplayTranscoder::create_with_physical_addresses(transcoder_registers_paddr, pipe_registers_paddr, dpll_registers_paddr, dpll_control_registers_paddr));
    m_planes[0] = MUST(IntelG33DisplayPlane::create_with_physical_address(m_mmio_first_region.pci_bar_paddr.offset(0x70180)));
    ...
    auto connector = IntelNativeDisplayConnector::must_create(*this, IntelNativeDisplayConnector::ConnectorIndex::PortA, IntelNativeDisplayConnector::Type::Analog, m_mmio_second_region.pci_bar_paddr, m_mmio_second_region.pci_bar_space_length);
...
bool IntelDisplayConnectorGroup::set_crt_resolution(DisplayConnector::ModeSetting const& mode_setting)
{
    ...
    MUST(m_planes[0]->set_aperture_base({}, gpu_aperture_vaddr));
