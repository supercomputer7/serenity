...
static bool check_pll_settings(const IntelGraphics::PLLSettings& settings, size_t reference_clock, const IntelGraphics::PLLMaxSettings& limits)
{
...
void IntelNativeDisplayConnector::write_to_register(IntelGraphics::RegisterIndex index, u32 value) const
{
    ...
    auto* reg = (volatile u32*)m_registers_region->vaddr().offset(to_underlying(index)).as_ptr();
    *reg = value;
}
...
void IntelNativeDisplayConnector::set_display_timings(const Graphics::Modesetting& modesetting)
{
    VERIFY(m_control_lock.is_locked());
    VERIFY(m_modeset_lock.is_locked());
