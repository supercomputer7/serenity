...
#include <Kernel/Bus/PCI/Forward.h>
...
public:
    static ErrorOr<NonnullRefPtr<Device>> from_enumerable_identifier(HostController&, EnumerableDeviceIdentifier const& identifier);
    static ErrorOr<NonnullRefPtr<Device>> from_host_bridge_identifier(EnumerableDeviceIdentifier const& identifier);
    ...
    inline ErrorOr<Memory::TypedMapping<T>> map_resource(HeaderType0BaseRegister bar, size_t size = sizeof(T), Memory::Region::Access access = IsConst<T> ? Memory::Region::Access::Read : Memory::Region::Access::ReadWrite)
    {
        ...
        auto bar_address = resource.physical_memory_address();
...
private:
    ...
    Device(HostController&, EnumerableDeviceIdentifier const& pci_identifier);

    // NOTE: This constructor is used for host bridge devices.
    ...
    // set it during construction for all PCI devices except for host bridges.
