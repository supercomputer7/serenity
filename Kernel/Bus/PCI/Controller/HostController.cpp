...
{
    // NOTE: Fix the reference of the host bridge PCI::Device to point
    // to this HostController now.
    m_root_bus->self_device().m_host_controller = const_cast<HostController&>(*this);
}
...
void HostController::fill_device_resources(PCI::Device& device)
{
    ...
    auto determine_device_resource_characteristics = [this](PCI::Device& device, PCI::Resource& resource, u8 field) -> void {
        ...
        resource.bar_value = bar_reserved;
    };
    ...
    for (size_t index = 0; index < device.m_resources.size(); index++) {
        ...
        if (resource.length != 0 && resource.bar_value != 0) {
            dbgln("{}: {} BAR {}, @ {:x} (length {})",
...
ErrorOr<void> HostController::enumerate_function(Bus& bus, DeviceNumber device, FunctionNumber function, bool recursive_search_into_bridges)
{
    ...
    auto enumerable_device = TRY(Device::from_enumerable_identifier(const_cast<HostController&>(*this), identifier));
    ...
    PCI::Access::register_device(*enumerable_device);
