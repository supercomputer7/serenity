...
extern "C" [[noreturn]] UNMAP_AFTER_INIT void init(BootInfo const& boot_info)
{
    ...
    if (!multiboot_framebuffer_addr.is_null()) {
        // If the bootloader provided a framebuffer then set up an initial console so we can see the output on the screen!
        boot_framebuffer_console = try_make_ref_counted<Graphics::BootFramebufferConsole>(multiboot_framebuffer_addr, multiboot_framebuffer_width, multiboot_framebuffer_height, multiboot_framebuffer_pitch).leak_ref();
    }
